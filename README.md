...
> ✨ **正式版已上线** —— 微信扫一扫即可体验
>
> <img src="docs/qrcode.jpg" width="200" alt="程序猿做饭指南小程序二维码" />
>
> 📱 微信扫一扫，或在微信中搜索「程序猿做饭指南」

基于开源菜谱仓库 [Anduin2017/HowToCook](https://github.com/Anduin2017/HowToCook)（约 360 道菜）实现的微信小程序。

## 项目简介

帮你解决「今天吃什么」和「怎么做」：按分类或搜索浏览约 360 道菜、查看分步做法；按**用餐人数 + 忌口 + 过敏原**一键推荐今日菜单，还能规划**一周不重样**的早 / 午 / 晚三餐。整体采用**蜡笔小新风格**（饱和蜡笔色 + 暖棕描边 + 贴纸投影），并内置 5 套可切换皮肤（含暗夜模式，已消除推页首帧闪白）。

📦 本仓库地址：https://github.com/shixiaotuo/HowToCook-xcx

## 更新日志

> 每次推送的更新内容在此汇总，便于追溯。

### 2026-08-31 · 正式版上线 · 二维码入仓
- **正式版已上线**，README 顶部将 `🚧 还在测试，待正式上线` 改为 `✨ 正式版已上线`
- **新增小程序二维码** `docs/qrcode.jpg`（344×344，67KB），README 顶部置顶展示
- 📱 微信扫一扫即可体验，亦可在微信中搜索「程序猿做饭指南」
...
- **全库餐单数据对照上游 HowToCook 统一修正**（360 道真菜，排除 18 道厨房技巧）：
  ...
  - 操作步骤全量重建 359 道，补齐 `## 子标题`、`> 说明`、`S<n>` 分段编号，与源一致
  - 附加内容补齐 306 道，链接转为裸 URL 便于长按复制，剔除图片 markdown 与页脚
- **用量计算保留全部规格**（如戚风蛋糕三规格：1 份 / 6 寸 / 8 寸）
- **详情页渲染完善**（`pages/detail`）：
  - 空食材 / 空用量增加 `wx:if` 防御，避免空卡片破版
  - 新增「附加内容」展示区块
  - 正文 `<text user-select="true">` 支持长按复制
- **新增同步脚本**：`scripts/sync_upstream.py` 等，支持按上游增量比对修复
