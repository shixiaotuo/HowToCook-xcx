...
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # cookbook-miniprogram
...
for cat in sorted(os.listdir(AI)):
    ...
    for name in sorted(os.listdir(cdir)):
        ...
        if not pngs:
            continue
        new_rel = f"/_ai_covers/{cat}/{name}/{pngs[0]}"
        old_jpg = os.path.join(OLD, cat, name, "cover.jpg")
        old_rel = f"/_covers_final/{cat}/{name}/cover.jpg" if os.path.exists(old_jpg) else None
        # 判断旧图来源
        src = "?"
        ...
        if old_rel:
            try:
                ...
                for it in data.get("recipes", data) if isinstance(data, dict) else data:
                    if it.get("category") == cat and it.get("name") == name:
                        src = it.get("source", "?")
                        break
            except Exception:
                pass
        ...
        old_img = f"<img loading='lazy' src='{old_rel}' onclick='zoom(this)'>" if old_rel else "<div class='empty'>无旧图</div>"
...
<figure class='new'><figcaption>AI 新图</figcaption><img loading='lazy' src='{new_rel}' onclick='zoom(this)'></figure>
</div>
...
<div class='meta'>共 {len(cards)} 道菜已用 AI 重新生成。左 = 原封面（真实图/PEXELS），右 = AI 生成新封面。点击任意图可放大。</div>
<div class='grid'>
{''.join(cards)}
</div>
